...
describe('rule-evaluator', () => {
  describe('evaluateRule', () => {
    it('should evaluate equals condition', () => {
      const rule = { param: 'value', condition: 'equals' as const, value: 'test' };
      expect(evaluateRule(rule, { value: 'test' })).toBe(true);
      expect(evaluateRule(rule, { value: 'other' })).toBe(false);
    });

    it('should evaluate contains condition', () => {
      const rule = { param: 'text', condition: 'contains' as const, value: 'test' };
    ...
    it('should evaluate greaterThan condition', () => {
      const rule = { param: 'amount', condition: 'greaterThan' as const, value: 100 };
  ...
  describe('evaluateParameterRules', () => {
    it('should evaluate multiple rules', () => {
      const rules = {
        testTool: [
          { param: 'amount', condition: 'greaterThan' as const, value: 100, securityLevel: SecurityLevel.HIGH },
          { param: 'type', condition: 'equals' as const, value: 'sensitive', securityLevel: SecurityLevel.CRITICAL },
        ],
      };

      const result = evaluateParameterRules('testTool', { amount: 150, type: 'sensitive' }, rules);
    ...
    it('should prioritize higher security levels', () => {
      const rules = {
        testTool: [
          { param: 'amount', condition: 'greaterThan' as const, value: 100, securityLevel: SecurityLevel.LOW },
          { param: 'amount', condition: 'greaterThan' as const, value: 1000, securityLevel: SecurityLevel.HIGH },
        ],
      };

      const result = evaluateParameterRules('testTool', { amount: 2000 }, rules);
  ...
  describe('RuleFactory', () => {
    ...
    it('should create patternMatches rule', () => {
      const rule = RuleFactory.patternMatches('email', '^[a-z]+@[a-z]+\\.com$', SecurityLevel.MEDIUM);
