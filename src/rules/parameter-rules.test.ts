...
describe('parameter-rules', () => {
  describe('CommonParameterRules', () => {
    ...
    describe('externalApi rules', () => {
      it('should define untrusted domain rule', () => {
        ...
        expect(rule.value).toBe('^(?!https://(api\\.trusted\\.com|api\\.partner\\.com))');
        expect(rule.securityLevel).toBe(SecurityLevel.MEDIUM);
      });
  ...
  describe('createRuleSet', () => {
    it('should create rule set with tool names', () => {
      const rules = [
        { param: 'amount', condition: 'greaterThan' as const, value: 100, toolName: 'PaymentTool' },
        { param: 'size', condition: 'lessThan' as const, value: 1000, toolName: 'FileTool' },
        { param: 'rate', condition: 'equals' as const, value: 0.5, toolName: 'PaymentTool' },
      ];
  ...
  describe('createPresetRuleSet', () => {
    ...
    it('should include correct rules for ProcessPayment', () => {
      ...
      expect(paymentRules).toContainEqual(CommonParameterRules.financial.highValueTransaction);
      expect(paymentRules).toContainEqual(CommonParameterRules.financial.criticalValueTransaction);
      expect(paymentRules).toContainEqual(CommonParameterRules.financial.unknownRecipient);
    });
    ...
    it('should include correct rules for AccessDatabase', () => {
      ...
      expect(databaseRules).toContainEqual(CommonParameterRules.dataAccess.sensitiveFields);
      expect(databaseRules).toContainEqual(CommonParameterRules.dataAccess.bulkAccess);
    });
    ...
    it('should include correct rules for CallExternalAPI', () => {
      ...
      expect(apiRules).toContainEqual(CommonParameterRules.externalApi.untrustedDomain);
      expect(apiRules).toContainEqual(CommonParameterRules.externalApi.highFrequency);
    });
    ...
    it('should include correct rules for FileOperation', () => {
      ...
      expect(fileRules).toContainEqual(CommonParameterRules.fileOperation.systemFiles);
      expect(fileRules).toContainEqual(CommonParameterRules.fileOperation.largeFile);
    });
  });
});