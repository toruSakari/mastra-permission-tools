...
describe('default-policy', () => {
  describe('defaultSecurityPolicy', () => {
    ...
    it('should have correct category security levels', () => {
      expect(defaultSecurityPolicy.categories.internal.securityLevel).toBe(SecurityLevel.NONE);
      expect(defaultSecurityPolicy.categories.read.securityLevel).toBe(SecurityLevel.LOW);
      expect(defaultSecurityPolicy.categories.write.securityLevel).toBe(SecurityLevel.MEDIUM);
      expect(defaultSecurityPolicy.categories.external.securityLevel).toBe(SecurityLevel.MEDIUM);
      expect(defaultSecurityPolicy.categories.financial.securityLevel).toBe(SecurityLevel.HIGH);
      expect(defaultSecurityPolicy.categories.admin.securityLevel).toBe(SecurityLevel.CRITICAL);
    });

    it('should have correct default security level settings', () => {
      expect(defaultSecurityPolicy.defaults[SecurityLevel.NONE].requirePermission).toBe(false);
      expect(defaultSecurityPolicy.defaults[SecurityLevel.LOW].requirePermission).toBe(true);
      expect(defaultSecurityPolicy.defaults[SecurityLevel.LOW].expiry).toBe('24h');
      expect(defaultSecurityPolicy.defaults[SecurityLevel.CRITICAL].expiry).toBe('once');
      expect(defaultSecurityPolicy.defaults[SecurityLevel.CRITICAL].requireConfirmation).toBe(true);
    });
  });

  describe('developmentSecurityPolicy', () => {
    it('should extend defaultSecurityPolicy with longer expiry times', () => {
      expect(developmentSecurityPolicy.defaults[SecurityLevel.HIGH].expiry).toBe('24h');
      expect(developmentSecurityPolicy.defaults[SecurityLevel.CRITICAL].expiry).toBe('1h');
      expect(developmentSecurityPolicy.categories).toEqual(defaultSecurityPolicy.categories);
    });
  });

  describe('restrictiveSecurityPolicy', () => {
    it('should have shorter expiry times than default', () => {
      expect(restrictiveSecurityPolicy.defaults[SecurityLevel.LOW].expiry).toBe('1h');
      expect(restrictiveSecurityPolicy.defaults[SecurityLevel.MEDIUM].expiry).toBe('30m');
      expect(restrictiveSecurityPolicy.defaults[SecurityLevel.HIGH].expiry).toBe('once');
      expect(restrictiveSecurityPolicy.defaults[SecurityLevel.HIGH].requireConfirmation).toBe(true);
    });
  });
  ...
  describe('createCustomPolicy', () => {
    it('should merge base policy with customizations', () => {
      ...
      expect(customPolicy.tools.CustomTool.securityLevel).toBe(SecurityLevel.HIGH);
      expect(customPolicy.categories.custom).toBeDefined();
      expect(customPolicy.categories.custom.securityLevel).toBe(SecurityLevel.MEDIUM);
    ...
    it('should override existing values', () => {
      ...
      expect(customPolicy.categories.read.securityLevel).toBe(SecurityLevel.HIGH);
      expect(customPolicy.categories.write.securityLevel).toBe(SecurityLevel.MEDIUM); // Unchanged
    });
  });
